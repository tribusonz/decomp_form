...
　指数関数は多価性を持つものではあるが，累乗根が解に現れるものもあり，リーマン球面に好まれる一次元な変数は射影に捉えることができていない。ただし，周期性$\mathi\pi$について自明であるのは有理関数近似を現すものとして有効である。  
...
　仮数部は全て無理数であり自然数は偶然の一致とするのが指数・対数の関係である。これの有利関数近似は興味深いほどシンメトリなのであって，指数にはないものは対数にはあって，逆に対数にないものは指数にはある。  
